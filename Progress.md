...
## 2026-07-23 — UI言語(i18n)対応 & 言語トグルの明確化

- **作業内容**（UI言語とお題言語を分離）:
  - **`G.ui`（UI表示言語）を新設**し、既存の `G.lang`（お題・入力言語）と**独立**に。これで「英語UI＋日本語入力→敵頭上のお題はひらがなのまま」が成立。
  - i18n辞書 `TXT={jp,en}` と `L(key)`、`applyUILang()` を追加。**メニュー/ヘルプ/ステージ選択/リザルト/ゲームオーバー**の全テキスト、**ヒート文言**（ダン→ダンダン→ダンダンダン!! ↔ BANG→BANG-BANG→BANG-BANG-BANG!!）、被弾「いてっ!!/OUCH!!」、回復、ボス出現、画面下ヒント、キーボードのヒント/濁点ガイドをUI言語で切替。
  - **タイトルロゴをUI言語連動**に：日本語UI＝「タイピングシューター」、英語UI＝「Typing Shooter」。
  - **タイトルのトグルを2段に整理**して分かりやすく：`言語 / Language`（UI言語）と `おだい / Words`（お題言語）。紛らわしい「ことば」表記を廃止。旧「スマホ入力(英字/かな)」トグル（`imRow`/`tgRoma`/`tgKana`）は撤去（お題=日本語ならPC=ローマ字/スマホ=フリック、お題=英語ならABCで自動）。
  - リザルト/ゲームオーバーは表示時に `L()` で生成、タイトル/ヘルプ/ステージ選択は `applyUILang()` で更新。未使用の `PUNCH_TXT` 定数を削除。
- **変更ファイル**: TypingShooter.html（`G.ui`、`TXT`/`L`/`applyUILang`、オーバーレイHTMLのID化と新トグル、`hitFx`/`damagePlayer`/`defeatEnemy`/`drawStateFx`/`drawBottomBar`/`renderPend`/`buildEnKbd`/`showResult`/`showGameOver`、トグル初期化）
- **検証**: `node --check` 構文OK。Node検証で jp/en の `L()` 値切替・`applyUILang()` 両言語正常・ロゴ切替を確認。
- **既定**: UI=日本語（ロゴ「タイピングシューター」）、お題=日本語。
- **次にやること**: 実機でトグルの分かりやすさ・英語UI時の各画面の収まりを確認。

## 2026-07-23 — 主人公を画像スプライト化 & コンボ持続ゲージ追加

- **作業内容**:
  - 主人公を `Soldier.png`（1254×1254/910KB/背景不透明の紫）から、Nodeで **紫背景を透過・キャラにトリム・268×240へ縮小** した RGBA PNG を **base64でHTML埋め込み**（外部素材なし維持、埋め込み後スプライト約37KB）。`SND` と同様に単一HTML内で完結。
  - `drawHero()` を画像描画に変更。銃口位置を自動検出（`HERO_MUZ={x:.994,y:.392}`）し、マズルフラッシュ/曳光弾の発射点 `MUZX/MUZY` に一致するよう配置。ボブ・リコイル・マズルフラッシュ演出は維持。`HERO_DH`(表示身長=79px)で調整可。画像未ロード時は従来のベクター絵 `drawHeroVec()` にフォールバック。
  - **コンボ持続ゲージ**を「n COMBO!」表示の下に追加。最後の正打から **950msで空**になる帯（緑→黄→赤）。空になると次の正打でコンボが1に仕切り直しになることを可視化（`drawHUD`）。
- **変更ファイル**: TypingShooter.html（`HERO_B64`/`HERO_MUZ`/`HERO_DH` 追加、`heroImg` ローダ、`drawHero`/`drawHeroVec`、`drawHUD` のコンボゲージ）。処理済み `hero_sprite.png` も出力（確認用、ゲームは埋め込み版使用）。
- **次にやること**: 実機で主人公の表示サイズ・立ち位置（`HERO_DH`）と銃口整合を確認。
